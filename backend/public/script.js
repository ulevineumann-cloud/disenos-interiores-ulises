...
function assertEl(el, id) {
  if (!el) {
    console.error(`Falta el elemento con id="${id}" en index.html`);
    alert(`Error: falta el elemento con id="${id}" en index.html`);
    return false;
  }
  return true;
}
...
function assertAll() {
  return (
    assertEl(boton, "generar") &&
    assertEl(estado, "estado") &&
    assertEl(loader, "loader") &&
    assertEl(modoInfo, "modoInfo") &&
    assertEl(recomendacionEl, "recomendacion") &&
    assertEl(imagenResultadoEl, "imagenResultado") &&
    assertEl(imagenOriginalEl, "imagenOriginal") &&
    assertEl(inputImagen, "imagen") &&
    assertEl(preview, "preview")
  );
}
...
if (boton) {
  boton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (data.modo) {
        modoInfo.textContent =
          data.modo === "IA_REAL" ? "🟢 Modo IA REAL" : "🔵 Modo DEMO (sin consumo de IA)";
      }
